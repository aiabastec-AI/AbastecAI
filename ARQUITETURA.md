...
- [ ] **BUG conhecido: mapa fica preto (sem tiles nem pins) em celular físico Samsung com GPU Xclipse** (encontrado em 2026-08-07, testado num Galaxy A56 / SM-A566E, Android 16, `@rnmapbox/maps` 10.3.5, New Architecture/Fabric ativa). Onboarding e toda a UI ao redor do mapa funcionam normalmente; o `GLRenderThread` do Mapbox inicializa sem erro (contexto EGL criado, "Native renderer created"), mas depois disso não aparece **nenhum log de carregamento de estilo ou tile** — nem sucesso nem erro, só silêncio — e a superfície GL nunca desenha nada. Conectividade confirmada (Supabase e Mapbox alcançáveis via `nc` direto do device). No emulador Android (renderização por software via `swiftshader`) o mesmo build carrega tiles e pins normalmente, então não é problema de dados/API/token — é específico do driver gráfico do aparelho físico. O log mostra `ANGLE Renderer: (Samsung Xclipse 540) on Vulkan 1.3.279`: a combinação Exynos+Xclipse com Mapbox GL Native via ANGLE/Vulkan tem histórico de bugs relatados no GitHub do `rnmapbox/maps` em outros aparelhos Samsung com essa GPU. Próximo passo, quando for investigar: tentar forçar backend OpenGL ES puro (sem passar pelo ANGLE/Vulkan) ou testar uma versão mais nova do `@rnmapbox/maps`. Não bloqueia a fase 1 em si (funciona no emulador e deve funcionar em aparelhos com GPU diferente), mas é um risco real pra produção — vale testar em mais um device físico (idealmente sem Xclipse) antes de considerar a fase 1 "pronta pra loja".
